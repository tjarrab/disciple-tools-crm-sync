...
powershell -NoProfile -Command "$plugin = [IO.Path]::GetFullPath($Env:PS_PLUGIN); $php = Join-Path $plugin 'disciple-tools-crm-sync.php'; $json = Join-Path $plugin 'version-control.json'; $line = (Get-Content $php | Where-Object { $_ -match 'Version:' } | Select-Object -First 1); if ($line -match 'Version:\s*(.+)') { $v = $Matches[1].Trim(); $j = Get-Content $json -Raw | ConvertFrom-Json; $j.version = $v; $j.download_url = ($j.download_url -replace '/download/[^/]+/disciple-tools-crm-sync[.]zip', ('/download/' + $v + '/disciple-tools-crm-sync.zip')); [IO.File]::WriteAllText($json, ($j | ConvertTo-Json -Depth 5), [Text.Encoding]::UTF8); Write-Host ('      Version: ' + $v) } else { Write-Error 'Could not parse Version from disciple-tools-crm-sync.php'; exit 1 }"
if %ERRORLEVEL% NEQ 0 (
    echo.
